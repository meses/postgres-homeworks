...
select distinct ship_country, ship_city from orders o
...
select avg(shipped_date - order_date) as count_days from orders o
...
select min(unit_price), max(unit_price) from products p
